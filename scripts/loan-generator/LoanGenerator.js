...
const getRandomLoanInput = (web3) => {
    const {toWei} = web3.utils;
    const auctionBlockLength = faker.random.number({min: 10000, max: 20000});
...
const deployRandomLoan = async (LoanDispatcher, web3, from) => {
    const input = getRandomLoanInput(web3);
...
const loanGenerator = async (numberLoans, web3) => {
    ...
    const response = await Promise.all(
       repeatFunction(numberLoans, deployRandomLoan, [LoanDispatcher, web3, accounts[0]])
    );
