...
const LoanContract = artifacts.require("LoanInstalments");
...
const LoanContractDispatcherContract = artifacts.require("LoanInstalmentsDispatcher");
...
describe("LoanInstalmentsDispatcher", () => {
  let DAIProxy;
  let DAIToken;
  let RaiseToken;
  let DepositRegistry;
  ...
  let LoanDispatcher;
  let SwapFactory;
  let accounts, lender, borrower, admin;
  ...
  before(async () => {
    ...
    try {
      ...
      loanTemplateAddress = (await LoanContract.new()).address;
    } catch (error) {
      throw error;
    }
  });
