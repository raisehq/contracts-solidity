...
function getDividend(instance, id) {
  return instance.dividends.call(id).then(function(obj) {
    return {
      id: obj[0].valueOf(),
      block: obj[1].valueOf(),
      time: obj[2].valueOf(),
...
function checkDividend(
  ...
  return new Promise(function(resolve, reject) {
    try {
      ...
      assert.equal(
        dividend.transferedBack,
        transferedBack,
        "dividend transfered back is not equal"
      );
...
function getEmission(instance, id) {
  return instance.emissions.call(id).then(function(obj) {
    return {
...
function getPhase(instance, id) {
  return instance.phases.call(id).then(function(obj) {
    if (obj.length === 3) {
      return {
...
async function testTransferFrom(contract, granter, grantee, to, tokens) {
  ...
  const trasnferFromResponse = await contract.transferFrom.call(granter, to, tokens + 1, {
    from: grantee
  });
  ...
  await checkState(
    ...
    {
      contract: {
        balanceOf: [{[granter]: granterBeforeBalance}, {[grantee]: granteeBeforeBalance}],
        allowance: {
          __val: tokens,
          _owner: granter,
          _spender: grantee
  ...
  const transferFromResponse = await contract.transferFrom.call(granter, to, tokens / 2, {
    from: grantee
  });
  ...
  await checkState(
    {contract},
    {
      contract: {
        balanceOf: [
          {
            [granter]: new BigNumber(granterBeforeBalance)
              .sub(new BigNumber(tokens).div(2))
              .valueOf()
          },
  ...
  if (Array.isArray(args)) {
    ...
    if (keys.length === 1) {
      ...
      assert.equal(
        val,
        args[keys[0]],
        `Contract ${contractId} state ${stateId} with arg ${
          keys[0]
        } & value ${val} is not equal to ${args[keys[0]]}`
      );
    ...
    assert.equal(
      val,
      args.__val,
      `Contract ${contractId} state ${stateId} with value ${val} is not equal to ${args.__val}`
    );
  } else {
    ...
    assert.equal(
      val,
      args,
      `Contract ${contractId} state ${stateId} with value ${val} is not equal to ${args.valueOf()}`
    );
  }
}
